...
def get_all_ratings():
    ratings = {}
    sources = [f"https://www.football-data.co.uk/mmz4281/{s}/{l}.csv" 
               for s in ["2425", "2324"] for l in ["E0", "F1", "SP1", "I1", "D1"]]
    for url in sources:
        try:
...
with tab2:
    ...
    if os.path.exists('historique_paris.csv'):
        ...
        if not clos.empty:
            p = sum([(r['Mise']*r['Cote']-r['Mise']) if r['Statut']==1 else -r['Mise'] for _,r in clos.iterrows()])
            st.metric("Profit Net", f"{p:.2f}€")
